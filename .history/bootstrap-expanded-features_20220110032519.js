...
async function createCSSRules(rule) {
  try {
    ...
    if (ruleOriginal)
      for (let [prop, val, newCSSText] of propsArr) {
        // ruleOriginal.style[prop] = val; is against the spec, and does not support !important.
        console.log(ruleOriginal);
        console.log(ruleOriginal.cssText);
        console.log(selector);
        console.log(prop);
        console.log(val);
        newCSSText = `${selector} { ${prop}: ${val}; }`;
        newCSSText = `${selector} { padding: 1rem; }`;
        console.log(newCSSText);
        await ruleOriginal.cssText = newCSSText;
        console.log(ruleOriginal.cssText);
        // ruleOriginal.style.setProperty(prop, ...val.split(/ *!(?=important)/));
      }
    else {
