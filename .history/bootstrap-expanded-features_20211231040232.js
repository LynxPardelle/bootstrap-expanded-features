        ...
        switch (bfeSplited[1]) {
          ...
          case "btn":
            if (value.includes(" OPA")) {
              ...
              /.${bfe}:hover{
                background-color: rgba(${await HexToRGB(
                  await shadeTintColor(await HexToRGB(colors[value]), -15)
                ).toString()}, ${value.split(" ")[2]});
              border-color: rgba(${await HexToRGB(
                await shadeTintColor(await HexToRGB(colors[value]), -20)
              ).toString()}, ${value.split(" ")[2]});}
              ...
              border-color: rgba(${await HexToRGB(
                await shadeTintColor(await HexToRGB(colors[value]), -25)
              ).toString()}, ${value.split(" ")[2]});
