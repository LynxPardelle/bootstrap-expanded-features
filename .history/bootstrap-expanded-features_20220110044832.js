...
async function cssCreate() {
  try {
    ...
    for (let bef of befs) {
      let befStringed = "." + bef;
      if(sheets.map((s) =>
      [...s.cssRules].reverse().find((i) => i.cssText.includes(befStringed))).filter((i) => i).pop()){
...
async function createCSSRules(rule) {
  try {
    ...
    if (ruleI && !ruleI.split("{")[0].includes("@media")) {
      ...
      ruleOriginal = await sheets.map(findRule).filter((i) => i).pop();
    } else {
      let i= 0;
      for (let ruleISplit of ruleI.split("{")) {
        ...
        selector = ruleISplit.includes("}") ? ruleISplit.split("}")[ruleISplit.split("}").length -1] : ruleISplit;
      // CSS (& HTML) reduce spaces in selector to one.
      selector = selector.replace('\n', '').replace(/\s+/g, " ");
      console.log(selector);
      let findRule = (s) =>
        [...s.cssRules].reverse().find((i) => i.cssText.includes(selector));
      ruleOriginal.push(await sheets.map(findRule).filter((i) => i).pop());
