        ...
        switch (bfeSplited[1]) {
          ...
          case "btn":
            if (value.includes(" OPA")) {
              ...
              /.${bfe}:hover{
                background-color: rgba(${await HexToRGB(
                  await shadeTintColor(await HexToRGB(colors[value.split(" ")[0]]), -15)
                ).toString()}, ${value.split(" ")[2]});
              border-color: rgba(${await HexToRGB(
                await shadeTintColor(await HexToRGB(colors[value.split(" ")[0]]), -20)
              ).toString()}, ${value.split(" ")[2]});}
              ...
              border-color: rgba(${await HexToRGB(
                await shadeTintColor(await HexToRGB(colors[value.split(" ")[0]]), -25)
              ).toString()}, ${value.split(" ")[2]});
              box-shadow: 0 0 0 0.25rem 
              rgba(${await HexToRGB(
                await shadeTintColor(await HexToRGB(colors[value.split(" ")[0]]), 3)
              )}, ${value.split(" ")[2]})
              ;}
