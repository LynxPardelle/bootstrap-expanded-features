...
async function createCSSRules(rule) {
  try {
    ...
    if (ruleI && !ruleI.split("{")[0].includes("@media")) {
      ...
      ruleOriginal = await [...sheet.cssRules].reverse().find(
        (i) => i.selectorText == selector
      );
      console.log(ruleOriginal);

      ruleOriginal = ruleOriginal ? await ruleOriginal.filter((i) => i) : undefined;
      console.log(ruleOriginal);

      ruleOriginal = ruleOriginal ? await ruleOriginal.pop() : undefined;
