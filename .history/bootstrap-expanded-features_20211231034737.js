        ...
        switch (bfeSplited[1]) {
          case "bg":
            if(value.includes(" OPA")){
              bfeStringed += `{background-color: rgba(${await HexToRGB(colors[value.split(' ')[0]]).toString()}, ${value.split(' ')[2]}) !important;}`;
            } else {
          ...
          case "text":
            if(value.includes(" OPA")){
              bfeStringed += `{color: rgba(${await HexToRGB(colors[value.split(' ')[0]]).toString()}, ${value.split(' ')[2]}) !important;}`;
            }else{
