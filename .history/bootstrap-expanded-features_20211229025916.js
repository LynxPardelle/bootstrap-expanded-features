    ...
    if (
      ...
      switch (bfeSplited[1]) {
        ...
        case "btn":
          ...
          /.${bfe}:hover{background-color:${await darken(await HexToRGB(colors[value]), 80)};border-color:${await darken(await HexToRGB(colors[value]), 80)}
          /.btn-check:focus + .${bfe}, .${bfe}:focus{background-color:${await darken(await HexToRGB(colors[value]), 80)};border-color:${await darken(await HexToRGB(colors[value]), 80)}
          /.btn-check:checked + .${bfe}, .btn-check:active + .${bfe}, .${bfe}:active, .${bfe}.active, .show > .${bfe}.dropdown-toggle{background-color:${await darken(await HexToRGB(colors[value]), 80)};border-color:${await darken(await HexToRGB(colors[value]), 80)}
          /.btn-check:checked + .btn-check:focus, .btn-check:active + .${bfe}:focus, .${bfe}:active:focus, .${bfe}.active:focus, .show > .${bfe}.dropdown-toggle:focus{box-shadow: 0 0 0 0.25rem ${await darken(await HexToRGB(colors[value]), 80)}}`;
          break;
      }
    }
...
function darken(rgb, darken){
  let newColor = "#";
  rgb.forEach((color)=>{newColor+=Math.round(color/(1-Math.pow(Math.E, -darken))).toString(16)});
  return newColor;
}
