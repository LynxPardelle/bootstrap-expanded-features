        ...
        Object.keys(values).forEach((v) => {
          ...
          if (!!hasOPA) {
            ...
            for (let OPA of OPAS) {
              ...
              let realColor = `${colorToRGB(
                !!colors[color.toString().replace(/\s/g, "")]
                  ? colors[color.toString().replace(/\s/g, "")]
                  : color
              ).toString()}`;
              values[v] = !!OPAValue
          ...
          if (!!matches) {
            for (let match of matches) {
              let realColor = colors[match.replace(/\s/g, "")];
              if (realColor) {
                values[v] = values[v].replace(
                  match,
                  `rgba(${colorToRGB(realColor)})`
                );
              }
            }
          }
      ...
      switch (true) {
        ...
        case befSplited[1] === "btn":
          befStringed += `{
                    ...
                    border-color:${value};}
                  ${separator}.${bef}:hover{background-color:rgba(${shadeTintColor(
            colorToRGB(value),
            -15
          ).toString()});border-color:rgba(${shadeTintColor(
            colorToRGB(value),
            -20
          ).toString()});}
                  ${separator}.btn-check:focus + .${bef}, .${bef}:focus{background-color:rgba(${shadeTintColor(
            colorToRGB(value),
            -15
          ).toString()});border-color:rgba(${shadeTintColor(
            colorToRGB(value),
            ...
            colorToRGB(value),
            -20
          )};border-color:rgba(${shadeTintColor(
            colorToRGB(value),
            -25
          ).toString()});box-shadow: 0 0 0 0.25rem
                  rgba(${shadeTintColor(colorToRGB(value), 3).toString()}, 0.5)
                  ;}
...
function colorToRGB(color) {
  try {
    ...
    switch (true) {
      case !!colors[color]:
        rgb = colorToRGB(colors[color]);
        break;
      case color.includes("rgb") || color.includes("rgba"):
        rgb = parseRGB(color);
        break;
      case color.includes("#"):
        rgb = parseRGB(HexToRGB(color));
        break;
      case color.includes("hsl"):
        rgb = parseRGB(HSLToRGB(color));
        break;
      case color.includes("hwb"):
        rgb = parseRGB(HWBToRGB(color));
        break;
      default:
        rgb = [255, 0, 0];
        break;
    ...
    consoleLog("error", { error: error }, styleConsole);
    return [255, 0, 0];
  }
}
...
function HexToRGB(Hex) {
  ...
  if (hexCodeLength === 3) {
    ...
    consoleLog("error", { hexToRGBError: "Invalid hex code" }, styleConsole);
  }
...
function removePseudos(thing, remove = false) {
  ...
  pseudoFiltereds.forEach((pse) => {
    ...
    switch (true) {
      case pseudosHasSDED.includes(pse.mask):
    ...
    thing = thing
      ...
      .replace(
        regMask,
        !remove
          ? pseudosHasSDED.includes(pse.mask)
            ? pse.real + "("
